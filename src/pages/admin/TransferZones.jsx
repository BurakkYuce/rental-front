...
const TransferZones = () => {
  ...
  const fetchZones = useCallback(async () => {
    try {
      ...
      const response = await fetch('http://localhost:4000/api/admin/transfers', {
        headers: {
  ...
  const handleUpdateZone = async (zoneId) => {
    try {
      const token = localStorage.getItem('admin_token');
      const response = await fetch(`http://localhost:4000/api/admin/transfers/${zoneId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  if (loading) {
    return (
      <AdminLayout title="Transfer Zones Management" subtitle="Manage transfer destinations and pricing">
        ...
        <table className="zones-table">
          ...
          <tbody>
            {zones.length === 0 ? (
              ...
              zones.map((zone) => (
                <tr key={zone.id} className={zone.status === 'inactive' ? 'inactive-zone' : ''}>
                  {editingZone === zone.id ? (
                    // Edit mode row
                    <>
                      <td>
                    ...
                    <>
                      ...
                      <td>
                        <span className={`status-badge ${zone.status}`}>
                          {zone.status === 'active' ? <i className="fa fa-eye"></i> : <i className="fa fa-eye-slash"></i>}
                          {zone.status}
                        </span>
                      </td>
                      <td>
                        <div className="action-buttons">
                          ...
                          <button
                            onClick={() => handleDeleteZone(zone.id, zone.zoneName)}
